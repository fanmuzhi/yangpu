...
void Yangpu::GetLogInfo(vector<string> listOfLogFilePath)
{
	...
	/*ui.tableWidget->setColumnCount(ListCounts);
	for (size_t t = 0; t < ListCounts; t++)
	{
		Syn_LogAnalyze *pSyn_LogAnalyze = new Syn_LogAnalyze();
		int rc = pSyn_LogAnalyze->LogAnalysis(listOfLogFilePath[t]);
		...
		{
			if (NULL != ui.tableWidget->item(i, t))
				ui.tableWidget->takeItem(i, t);
			if (NULL != ui.tableWidget->cellWidget(i, t))
				ui.tableWidget->removeCellWidget(i, t);
		}
		...
		if (0 != pLogResults->SensorSerialNumber.size())
		{
			QTableWidgetItem *itemSerialNumber = new QTableWidgetItem(QString::fromStdString(pLogResults->SensorSerialNumber));
			itemSerialNumber->setTextAlignment(Qt::AlignCenter);
			ui.tableWidget->setItem(0, t, itemSerialNumber);
		}

		if (0 != pLogResults->SW_Version.size())
		{
			QTableWidgetItem *itemVersion = new QTableWidgetItem(QString::fromStdString(pLogResults->SW_Version));
			itemVersion->setTextAlignment(Qt::AlignCenter);
			ui.tableWidget->setItem(1, t, itemVersion);
		}

		if (0 != pLogResults->ConfigFilePath.size())
		{
			QTableWidgetItem *itemConfigFile = new QTableWidgetItem(QString::fromStdString(pLogResults->ConfigFilePath));
			ui.tableWidget->setItem(2, t, itemConfigFile);
		}
		...
		if (pLogResults->SNRTestResult.bExcuted)
		{
			QTableWidgetItem *itemSignal = new QTableWidgetItem(QString::number(pLogResults->SNRTestResult.listOfSignalValue[pLogResults->SNRTestResult.listOfSignalValue.size() - 1]));
			itemSignal->setTextAlignment(Qt::AlignCenter);
			ui.tableWidget->setItem(3, t, itemSignal);

			QTableWidgetItem *itemNoise = new QTableWidgetItem(QString::number(pLogResults->SNRTestResult.listOfNoiseValue[pLogResults->SNRTestResult.listOfNoiseValue.size() - 1]));
			itemNoise->setTextAlignment(Qt::AlignCenter);
			ui.tableWidget->setItem(4, t, itemNoise);

			QTableWidgetItem *itemSNR = new QTableWidgetItem(QString::number(pLogResults->SNRTestResult.listOfSNRlValue[pLogResults->SNRTestResult.listOfSNRlValue.size() - 1]));
			itemSNR->setTextAlignment(Qt::AlignCenter);
			ui.tableWidget->setItem(5, t, itemSNR);
		}
		...
		if (0 != pLogResults->listOfBinCode.size())
		{
			...
			QTableWidgetItem *itemBindeCodes = new QTableWidgetItem(QString::fromStdString(strBinCodesValue));
			itemBindeCodes->setTextAlignment(Qt::AlignCenter);
			ui.tableWidget->setItem(6, t, itemBindeCodes);
		}

		if (pLogResults->NoFingerResult.bExcuted)
		{
			int rowNumber = pLogResults->NoFingerResult.RowNumber;
			int columnNumber = pLogResults->NoFingerResult.ColumnNumber;
			...
			for (int m = 0; m < rowNumber; m++)
			{
				...
				{
					data[k] = pLogResults->NoFingerResult.arr_NoFinger[m][n];
					k++;
				}
			}
			...
			ui.tableWidget->setCellWidget(7, t, pImageLabel);
			ui.tableWidget->resizeRowToContents(7);
		}

		if (pLogResults->FakeFingerResult.bExcuted)
		{
			int rowNumber = pLogResults->FakeFingerResult.RowNumber;
			int columnNumber = pLogResults->FakeFingerResult.ColumnNumber;
			QVector<QRgb> vcolorTable;
			for (int i = 0; i < 256; i++)
				vcolorTable.append(qRgb(i, i, i));
			QByteArray data;
			...
			for (int m = 0; m < rowNumber; m++)
			{
				for (int n = 0; n < columnNumber; n++)
				{
					data[k] = pLogResults->FakeFingerResult.arr_FakeFinger[m][n];
					k++;
				}
			}
			...
			pImageLabel->setPixmap(QPixmap::fromImage(image));
			pImageLabel->setAlignment(Qt::AlignCenter);
			ui.tableWidget->setCellWidget(8, t, pImageLabel);
